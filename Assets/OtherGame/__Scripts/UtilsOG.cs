...
public class UtilsOG : MonoBehaviour {
...
//============================ Bounds Functions ============================\
	...
	public static Vector3 ScreenBoundsCheck(Bounds bnd, BoundsTestOG test = BoundsTestOG.center) {
		// Call the more generic BoundsInBoundsCheck with camBounds as bigB
		return( BoundsInBoundsCheck( camBounds, bnd, test ) );
	}
	
	// Tests to see whether lilB is inside bigB
	public static Vector3 BoundsInBoundsCheck( Bounds bigB, Bounds lilB, BoundsTestOG test = BoundsTestOG.onScreen ) {
...
public class EasingOG {
	...
	static public Dictionary<string,EasingCachedCurveOG> cache;
	...
	//   million unique EasingOG parameters are called
	
	static public float Ease( float u, params string[] curveParams ) {
		// Set up the cache for curves
		if (cache == null) {
			cache = new Dictionary<string, EasingCachedCurveOG>();
		}
	...
	static private void EaseParse( string curveIn ) {
		EasingCachedCurveOG ecc = new EasingCachedCurveOG();
		// It's possible to pass in several comma-separated curves
	...
	static private float EaseP( float u, EasingCachedCurveOG ec ) {
		float u2 = u;
